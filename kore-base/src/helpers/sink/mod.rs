...
use crate::{model::event::{Ledger, LedgerValue}, EventRequest, Signed};
...
impl Subscriber<Signed<Ledger>> for KoreSink {
    async fn notify(&self, event: Signed<Ledger>) {
        ...
        let EventRequest::Fact(event) = event.content.event_request.content else {
            return;
        };

        let client = reqwest::Client::new();
        match client.post(format!("{}/{}", self.sink.clone(), subject_id)).json(&event.payload.0).send().await {
            Ok(res) => {
                if let Err(e) = res.error_for_status() {
                    error!(TARGET_SINK, "The information was not sent to the sink: {}", e); 
                } else {
