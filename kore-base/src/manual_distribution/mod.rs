...
const TARGET_MANUAL_UPDATE: &str = "Kore-Node-ManualUpdate";
...
impl ManualUpdate {
    ...
    async fn get_last_ledger(
        ctx: &mut ActorContext<ManualUpdate>,
...
pub enum ManualUpdateMessage {
    Update(DigestIdentifier)
}

impl Message for ManualUpdateMessage {}

#[async_trait]
impl Actor for ManualUpdate {
    type Message = ManualUpdateMessage;
    type Event = ();
    type Response = ();
...
#[async_trait]
impl Handler<ManualUpdate> for ManualUpdate {
    async fn handle_message(
        &mut self,
        _sender: ActorPath,
        msg: ManualUpdateMessage,
        ctx: &mut actor::ActorContext<ManualUpdate>,
    ) -> Result<(), ActorError> {
        match msg {
            ManualUpdateMessage::Update(subject_id) => {
                ...
                let Some(last_event) = last_event else {
                    ...
                    error!(TARGET_MANUAL_UPDATE, "Update, {}", e);
                    ...
                    if let EventProof::Transfer { .. } = last_proof.event {
                        ...
                        if verify {
                            if self.our_key != last_proof.owner {
                                ...
                                warn!(TARGET_MANUAL_UPDATE, "Update, {}", e);
                ...
                let distribution_actor = ctx.create_child(&request_id, distribution).await.map_err(|e| {
                    warn!(TARGET_MANUAL_UPDATE, "Update, Can not create distribution child: {}", e);
    ...
    async fn on_child_fault(
        ...
        error!(TARGET_MANUAL_UPDATE, "OnChildFault, {}", error);
