...
use super::{HashId, signature::{Signature, Signed}, wrapper::ValueWrapper};
...
impl HashId for EventRequest {
    fn hash_id(&self, derivator: DigestDerivator) -> Result<DigestIdentifier, Error> {
        DigestIdentifier::from_serializable_borsh(self, derivator).map_err(|_| {
            Error::Signature("HashId for EventRequest Fails".to_string())
        })
    }
}
...
impl TryFrom<Signed<EventRequest>> for KoreRequest {
    ...
    fn try_from(event_request: Signed<EventRequest>) -> Result<Self, Self::Error> {
        let id = DigestIdentifier::generate_with_blake3(&event_request)
            .map_err(|_| Error::Digest("Error generation request hash".to_owned()))?;
        let subject_id = match &event_request.content {
            EventRequest::Create(_) => None,
            EventRequest::Fact(fact_request) => Some(fact_request.subject_id.clone()),
            EventRequest::Transfer(transfer_res) => Some(transfer_res.subject_id.clone()),
            EventRequest::EOL(eol_request) => Some(eol_request.subject_id.clone()),
        };
        Ok(Self {
            id,
            subject_id,
