...
async fn test_governance_and_subject_copy_with_approve() {
    ...
    wait_for_governance_sync(governance_id.clone(), &[node1, node2], 5, 1, 3)
        .await
        .unwrap();
...
#[tokio::test]
#[tracing_test::traced_test]
async fn test_namespace_in_approvation() {
    // Bootstrap ≤- 2xAddressable
    let nodes = create_nodes_and_connections(
        vec![vec![]],
        vec![vec![0], vec![0]],
        vec![],
...
#[tokio::test]
#[tracing_test::traced_test]
async fn test_modify_init_state() {
    let nodes = create_nodes_and_connections(
        vec![vec![]],
        vec![vec![0]],
        vec![],
