...
use tell::{
    binary, Event as TellEvent, InboundFailure, InboundTellId, OutboundFailure,
    OutboundTellId, ProtocolSupport, TellMessage,
};
    ...
    pub fn new(
        ...
        let (dcutr, relay_server) = match config.node_type {
            NodeType::Bootstrap { .. } => {
                ...
                let relay_server =
                    RelayServer::new(peer_id, Default::default());
...
mod tests {
    ...
    async fn test_relay() {
        ...
        let config =
            create_config(boot_nodes.clone(), false, NodeType::Bootstrap);
        ...
        let established_conn_id = node_b
            .wait(move |e| match e {
                SwarmEvent::ConnectionEstablished {
                    ...
                    connection_id,
                    ..
                } => (*endpoint.get_remote_address() == node_a_addr)
        ...
        let loop_a = async move {
            loop {
                match node_a.select_next_some().await {
                    ...
                    SwarmEvent::Behaviour(Event::PeersFounded(
                        _,
                        peers_founded,
                    )) => {
            ...
            loop {
                match node_b.select_next_some().await {
                    ...
                    SwarmEvent::Behaviour(Event::Dcutr(dcutr::Event {
                        ...
                        node_b.behaviour_mut().send_message(
                            &node_a_peer_id,
                            b"Hello Node A".to_vec(),
                        );
                    }
                    SwarmEvent::Behaviour(Event::MessageSent {
                        peer_id,
                        ..
                    }) => {
                        // **** Message sent ****
                ...
                match node_a.select_next_some().await {
                    ...
                    SwarmEvent::Behaviour(Event::MessageSent {
                        ...
                        if reservation.is_none() {
                            ...
                            let addresses = node_a
                                .behaviour_mut()
                                .addresses_of_peer(&boot_node_peer_id);
                            if addresses.is_empty() {
                                // Discover node b.
                                node_a
                                    .behaviour_mut()
                                    .discover(&boot_node_peer_id)
                            } else {
                                if let Some(base_addr) = addresses.iter().next()
                                {
                                    ...
                                    if let Ok(addr) =
                                        addr.with_p2p(boot_node_peer_id)
                                    {
                                        let listen_addr =
                    ...
                    SwarmEvent::Behaviour(Event::MessageProcessed {
                        peer_id,
                        ..
                    }) => {
                ...
                match node_b.select_next_some().await {
                    SwarmEvent::Behaviour(Event::Identified {
                        ...
                        if let Some(relay_addr) =
                            node_b.behaviour().get_relay_node(&peer_id)
                        {
                            assert_eq!(peer_id, node_a_peer_id);
    ...
    async fn test_reservation_error() {
        ...
        let fake_addr =
            Multiaddr::from_str("/ip4/127.0.0.1/tcp/11111").unwrap();
