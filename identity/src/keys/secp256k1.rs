...
impl KeyMaterial for Secp256k1KeyPair {
    ...
    fn to_secret_der(&self) -> Result<Vec<u8>, Error> {
        ...
        let signing_key = SigningKey::try_from(secret_bytes.as_slice())
            .map_err(|_| {
                Error::KeyPair("Cannot generate signing key".into())
            })?;
        ...
        Self: Sized,
    {
        match kp_type {
            KeyPairType::Secp256k1 => {
                let secret_key =
                    SecretKey::from_pkcs8_der(der).map_err(|_| {
                        Error::KeyPair("pkcs8 deserialize error".into())
                    })?;
        ...
        let verified = match payload {
            Payload::Buffer(payload) => {
                ...
                let signature = Signature::from_slice(signature)
                    .expect("Couldn't parse signature");
...
mod tests {
    ...
    fn test_ser_des() {
        ...
        let new_kp: Result<Secp256k1KeyPair, serde_json::Error> =
    ...
    fn test_sign_verify_fail() {
        ...
        let signature = kp.sign(Payload::Buffer(msg.to_vec())).unwrap();
        let result =
            kp.verify(Payload::Buffer(b"message2".to_vec()), &signature);
    ...
    fn test_der() {
        ...
        let new_kp =
            Secp256k1KeyPair::from_secret_der(KeyPairType::Secp256k1, &der)
                .unwrap();
